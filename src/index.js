...
function buildMarketEmbed(market, odds, closed = false) {
  ...
  const choiceLines  = market.choices.map((choice, i) => {
    ...
    return `**${choice}**\n\`${buildProgressBar(ratio)}\` ${Math.round(ratio * 100)}% — cote **x${odds[i]}**\n📊 ${bettors} parieurs · ${amount} pts misés`;
  });
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: '📦 Total misé', value: `${totalBets} pts par ${totalBettors} parieurs`, inline: true },
      { name: '⏰ Fermeture', value: `<t:${Math.floor(closeDate.getTime() / 1000)}:R>`, inline: true }
    );
...
async function refreshMarketMessage(client, marketId) {
  ...
  } catch (err) { console.error('❌ Erreur refresh embed :', err.message); }
}
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (interaction.commandName === 'give-points') {
      ...
      return interaction.reply({
        embeds: [new EmbedBuilder()
          .setDescription(`✅ **${amount} pts** crédités à <@${target.id}>. Nouveau solde : **${getBalance(target.id)} pts**`)
          .setColor('#3498db')],
        ephemeral: true,
      });
    }
    ...
    if (interaction.commandName === 'create-market') {
      ...
      if (isNaN(closeDate.getTime()) || closeDate <= new Date())
        return interaction.reply({ content: '❌ Date invalide. Format : `2026-06-24T21:00:00`', ephemeral: true });
      const marketId = `mkt_${Date.now()}`;
      const market = { id: marketId, title, choices: [c1, c2, c3], closeAt: closeDate.toISOString(), closed: false, image, channelId: channel.id, messageId: null, bets: { 0: [], 1: [], 2: [] } };
      const odds = computeOdds(market);
      ...
      if (ADMIN_LOG_CHANNEL) {
        try {
          ...
          await logCh.send({ embeds: [new EmbedBuilder().setTitle('📋 Nouveau marché créé').setDescription(`**Titre :** ${title}\n**ID :** \`${marketId}\`\n**Fermeture :** <t:${Math.floor(closeDate.getTime()/1000)}:F>\n**Channel :** <#${channel.id}>`).setColor('#3498db')] });
        } catch {}
      }
    ...
    if (interaction.commandName === 'set-market-result') {
      ...
      const winnerIndex = interaction.options.getInteger('gagnant') - 1;
      const markets = loadMarkets();
      const market  = markets[marketId];
      ...
      try {
        ...
        await ch.send({ embeds: [new EmbedBuilder().setTitle('🏆 Résultat du marché').setDescription(`**${market.title}**\n\n✅ Choix gagnant : **${market.choices[winnerIndex]}** (x${winnerOdd})\n\n${winners.length} gagnant(s) — **${totalCredited} pts** crédités.`).setColor('#f1c40f')] });
      } catch {}
    ...
    if (interaction.commandName === 'close-market') {
      ...
      saveMarkets(markets);
      await refreshMarketMessage(client, marketId);
      return interaction.reply({ content: `✅ Marché \`${marketId}\` fermé.`, ephemeral: true });
    }
  }
  ...
  if (interaction.isButton()) {
    ...
    if (balance < 1)
      return interaction.reply({ content: "❌ Tu n'as pas assez de points pour parier.", ephemeral: true });
    const odds       = computeOdds(market);
    ...
    const input = new TextInputBuilder()
      .setCustomId('amount')
      .setLabel(`Cote : x${currentOdd} | Solde : ${balance} pts`)
...
client.once('ready', () => {
  ...
  console.log(`🧪 TEST_MODE : ${TEST_MODE ? 'activé (paris multiples)' : 'désactivé (1 pari par marché)'}`);
  const markets = loadMarkets();
